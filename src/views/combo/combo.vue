  ...
  <div class="combolayout deleteflex">
    <header>
      <div class="headerbutton">
        <div style="display: inline-block">
          <h2>
            体检套餐选择
            <span style="font-size: 16px">(关联套餐)</span>
          </h2>
    ...
    <section style="-webkit-overflow-scrolling:touch">
      ...
      <div
        ...
        @click="combodetails(item.TCID00)"
        :key="item"
        v-show="item.SYNA00 === '1' && model2 === '男' || item.SYNV00 === '1' && model2 === '女' || model2 === ''"
      >
        <img src="../../assets/logo.png" alt class="itemicon">
        <div class="itemcontent">
          <p style="font-weight: bold;">{{item.TCMC00}}</p>
          <p>适 用：{{item.SYNV00 === '0' ? '男': (item.SYNA00 === '0'? '女': '所有')}}</p>
          <p>
            预估价：
            <span style="color: red">￥{{item.TCJG00}}</span>
          </p>
        </div>
      </div>
      <div class="comboitem" v-for="item in 7" :key="item" v-show="comboList.length === 0">
        <img src="../../assets/logo.png" alt class="itemicon">
        <div class="itemcontent">
          <div>
            <div class="loading-img" style="height:80%;">
              <div class="infiniteblock"></div>
            </div>
          </div>
          <div>
            <div class="loading-img" style="height:80%;">
    ...
    <section>
      <Modal v-model="showtype" title="体检明细列表" footer-hide="true" closable="true" class="pemodal">
        <div class="petype">
          <div class="petype1" style="display:inline-block;">
            <Icon type="ios-bulb" size="48"/>
            <div style="width:72px;">入职入学体检</div>
          </div>
          <div style="display: inline-block;">
            ...
            <div class="petype3" style="height:150px;">
              ...
              <div style="width:90px;">公务员招聘体检</div>
            </div>
          </div>
          <div style="display: inline-block;">
...
import { getGdtcList, getGdtcItemList } from '../../service/service';
...
export default {
  ...
  mounted() {
    getGdtcList('yyid00=222667&xb0000=0')
...
.loading-img {
  background: linear-gradient(to right, rgb(245, 247, 249), white);
}
.infiniteblock {
  height: 100%;
  width: 20%;
  background: linear-gradient(to right, rgb(245, 247, 249), white);
  animation: 1s linear 0s infinite alternate baba;
}
</style>
