  ...
  <div class="ordercontent">
    <header class="searchbar">
      ...
      <div class="orderright">
        ...
        <div style="display: inline-block;">
          <Input type="text" search v-model="searchname" placeholder="按姓名检索"/>
        </div>
      </div>
    </header>
        ...
        <div class="itemtop">
          <div class="topbage">
            <div v-if="item.TTID00 === '0'">
              <span style="margin-left:0.5px;">个人</span>
              <br>订单
            </div>
...
export default {
  ...
  created() {
    gettjzxx(
      ...
      .then((res) => {
        const response = res.responseEntity.entity;
        // alert(this.yourorder[1].DJRQSJ);
        // alert('123'.toString().replace('1', '2'));
        for (let i = 0; i < response.length; i++) {
          const temp = response[i].DJRQSJ.split('');
          for (let j = 0; j < temp.length; j++) {
            if (temp[j] === '-') {
              temp[j] = '/';
            }
          }
          response[i].DJRQSJ = temp.join('');
        }
        this.yourorder = response;
      });
  },
  methods: {
    getorder(item) {
