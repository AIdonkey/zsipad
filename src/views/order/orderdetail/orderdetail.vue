<template>
    <div class="ordercontent">
        ...
        <section class="ordersection">
            <div class="orderitem" v-for="(item, index) in yourorder" :key="index" @click="getorder(item)" v-show="Date.parse(date1) <= Date.parse(item.DJRQSJ) && Date.parse(date2) >= Date.parse(item.DJRQSJ) && item.XM0000.indexOf(searchname) !== -1">
                <div class="itemtop">
                    <div class="topbage">
                        <div v-if="item.TTID00 === '0'">
                            个人<br>
                            订单
                        </div>
                        <div v-if="item.TTID00 === '1'">
                           团体<br>
                           订单
                        </div>
                    </div>
...
export default {
  ...
  created() {
    gettjzxx(`yyid00=${this.hospital}&zjbh00=${this.personinfo.zjbh00}&xm0000=${encodeURI(encodeURI(this.personinfo.xm0000))}&choose=20`).then(response => response.json()).then((res) => { this.yourorder = res.responseEntity.entity; });
  },
  methods: {
    getorder(item) {
      this.$router.push(`/home/combodetail?tjh000=${item.TJH000}&tjyyrq=${item.TJYYRQ}`);
    },
  },
};
</script>
